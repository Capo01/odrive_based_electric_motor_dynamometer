...
def report_motor_parameters():
    ...
    absorber_motor_temperature = absorber_motor_odrive.axis0.muv4
    ...
    print('Absorber motor Temperature (Dec): ', absorber_motor_temperature)
    ...
    with open(text_file_name_motor_param,"a+") as text_file:
        ...
        text_file.write('Absorber motor temperature (Dec) ,' + str(absorber_motor_temperature) + '\n')
        ...
        text_file.write('Test motor temperature (Dec) ,' + str(test_motor_temperature) + '\n')

    print("### Finished reporting motor parameters ###")
     
def no_load_speed_test(odrive):
    ...
    """
    odrive.axis0.controller.config.vel_limit = rpm_to_cpr(no_load_max_motor_speed) # Limit motor top speed [counts/s]
    odrive.axis0.controller.config.control_mode = CTRL_MODE_VELOCITY_CONTROL  # set to velocity control mode
    odrive.axis0.controller.vel_setpoint = 0 # Stop motor if it isn't already stopped [counts/s]
    while odrive.axis0.encoder.vel_estimate != 0:# wait for motors to slow down.
        time.sleep(0.1)
    ...
    while vel_error < no_load_max_error:
        ...
        odrive.axis0.controller.vel_setpoint = rpm_to_cpr(set_speed) # [counts/s]
...
def motor_controller_loss_test(odrive):
    """
    ...
    for i in range(loss_est_num_steps):
        ...
        while odrive.axis0.current_state != AXIS_STATE_IDLE: # wait for offset calibration to finish before incrementing current 
            if x == 0:
                print(i, ' ', odrive.axis0.motor.config.calibration_current, ' ', motor_temp(loss_est_motor_under_test))
                time.sleep(stabilise_time)
                write_values(measure_values())
                x = 1 # Write to file only once per calibration current setting
            time.sleep(0.5)

    odrive.axis0.motor.config.calibration_current = calibration_current # set calibration current back to default value.
